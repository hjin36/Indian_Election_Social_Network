...
focal_likelihood <- border_with_new_party_plus_focal[,.(existed_likelihood = sum(focal_window > window & focal_party == party_name,na.rm = TRUE) / .N, non_existed_likelihood = 1 - sum(focal_window > window & focal_party == party_name,na.rm = TRUE) / .N),by = .(focal_district,focal_window)]
